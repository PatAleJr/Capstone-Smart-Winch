...
def fit_cord(cord: CordRecords.Cord, num_jumps: int):
    ...
    return least_squares(get_errors, x0=x, diff_step=1e-2, args=(training_set_of_jumps, cord), bounds=bounds)

### Validation
def validate_cord(cord: CordRecords.Cord, fitting_params, num_jumps: int, num_worst_jumps_to_print: int = 0):
    ...
    sum_of_squared_errors = get_errors(fitting_params, validation_set_of_jumps, cord) ** 2
    sum_of_absolute_errors = np.abs(get_errors(fitting_params, validation_set_of_jumps, cord))
...
def fit_and_validate_cord(cord: CordRecords.Cord, num_training_jumps: int, num_validation_jumps: int):
    fit_result = fit_cord(cord, num_training_jumps)
    MAE, MSE = validate_cord(cord, fit_result.x, num_validation_jumps)
